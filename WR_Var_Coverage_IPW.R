...
#############################################################
######### Variance Coverage of IPW estimator###########
#############################################################
...
for (count_temp in 1:sim_num){
  ...
  results_IPW = apply(matrix(1:n_count,nrow=1),2,compare_rows_IPW)
  ...
  WD_sim_IPW[count_temp] <- tau1_IPW - tau2_IPW
  ...
  #results_g1_IPW_trt <- mclapply(1:n, g1_IPW_trt_fun,df_comb$outcomes_comb,df_comb$treatment, pi_func, mc.cores = numCores-1)
  ...
  #                                df_comb$treatment, pi_func, mc.cores = numCores)
  ...
  ### B ### (Dim: 7*1): 6 covariates+intercept
  dBeta_B <- function() {
    n <- nrow(df_comb)
  ...
  ###A Treatment### (Dim: 7*1)
  calculate_An_IPW = function(df_comb,pi_func,adjusted_sum_i_neq_j,dBeta_B_val,dBeta_C_val){
    covariate_names <- c("x1", "x2", "x3", "x4", "x5", "x6")
    n <- nrow(df_comb)
    An_first_f = function(i){
      Xi <- c(1,as.matrix(df_comb[i, covariate_names])) #plus intercept
      numerator_vec1 <- numerator_vec2 <- numeric(length(covariate_names)+1)
      for(j in 1:n){
        Xj <- c(1,as.matrix(df_comb[j, covariate_names]))
        e_Xi <- pi_func[i]
        e_Xj <- pi_func[j]
        term1 <- (e_Xj * Xj - (1 - e_Xi) * Xi) * (df_comb$treatment[i] * (1 - df_comb$treatment[j])) *
          as.numeric(df_comb$outcomes[i] > df_comb$outcomes[j]) / (e_Xi * (1 - e_Xj))
        term2 <- (e_Xj * Xj - (1 - e_Xi) * Xi) * (df_comb$treatment[i] * (1 - df_comb$treatment[j])) *
          as.numeric(df_comb$outcomes[i] < df_comb$outcomes[j]) / (e_Xi * (1 - e_Xj))
    ...
    An_trt = An_first_trt-dBeta_B_val*(WP_trt_sim_IPW[count_temp]/dBeta_C_val)
    An_ctrl = An_first_ctrl-dBeta_B_val*(WP_ctrl_sim_IPW[count_temp]/dBeta_C_val)
    return (list(An_trt=An_trt,An_ctrl=An_ctrl))
  }
  An_IPW = calculate_An_IPW(df_comb,pi_func,adjusted_sum_i_neq_j,dBeta_B_val,dBeta_C_val)
  ...
  IPW_Variance_trt <- function() {
    ...
    An_t <- t(An_trt_IPW)
    for (i in 1:n) {
      var_sum <- var_sum + (2 * (g1_IPW_trt[i] - WP_trt_sim_IPW[count_temp]) + An_t %*% l_beta_fun(i))^2
    }
  ...
  IPW_Variance_ctrl <- function() {
    ...
    An_t <- t(An_ctrl_IPW)
    for (i in 1:n) {
      var_sum <- var_sum + (2 * (g1_IPW_ctrl[i] - WP_ctrl_sim_IPW[count_temp]) + An_t %*% l_beta_fun(i))^2
    }
  ...
  theory_var_WR[count_temp] <- var_ratio(WP_trt_sim_IPW[count_temp],WP_ctrl_sim_IPW[count_temp],
                                         theory_var_IPW_trt[count_temp],theory_var_IPW_ctrl[count_temp],
                                         Cov_trt_ctrl[count_temp])
  theory_var_WD[count_temp] <- Var_trt_IPW_calculated+Var_ctrl_IPW_calculated-2*cov_IPW_calculated
}#open on line
end_time <- Sys.time()
...
theory_sd_IPW_trt <- sqrt(theory_var_IPW_trt)
theory_sd_IPW_ctrl <- sqrt(theory_var_IPW_ctrl)
theory_sd_WR = sqrt(theory_var_WR)
theory_sd_WD = sqrt(theory_var_WD)

WP_trt_IPW_coverage <- mean((WP_trt_sim_true > (WP_trt_sim_IPW - qnorm(0.975)*theory_sd_IPW_trt)) & 
                              (WP_trt_sim_true < (WP_trt_sim_IPW + qnorm(0.975)*theory_sd_IPW_trt)))
WP_ctrl_IPW_coverage <- mean((WP_ctrl_sim_true > (WP_ctrl_sim_IPW - qnorm(0.975)*theory_sd_IPW_ctrl)) & 
                               (WP_ctrl_sim_true < (WP_ctrl_sim_IPW + qnorm(0.975)*theory_sd_IPW_ctrl)))
WR_coverage <- mean((WR_sim_true > (WR_sim_IPW - qnorm(0.975)*theory_sd_WR)) & 
                      (WR_sim_true < (WR_sim_IPW + qnorm(0.975)*theory_sd_WR)))
WD_coverage <- mean((WD_sim_true > (WD_sim_IPW - qnorm(0.975)*theory_sd_WD)) & 
                      (WD_sim_true < (WD_sim_IPW + qnorm(0.975)*theory_sd_WD)))

all_est = cbind(WP_trt_sim_IPW,WP_ctrl_sim_IPW,WR_sim_IPW,WD_sim_IPW)
wp_true = c(WP_trt_sim_true,WP_ctrl_sim_true,WR_sim_true,WD_sim_true)
wp_est = apply(all_est,2,mean)
wp_sd = apply(all_est,2,sd)
wp_se = sqrt(apply(cbind(theory_var_IPW_trt,theory_var_IPW_ctrl,theory_var_WR,theory_var_WD),2,mean))
wp_cr = c(WP_trt_IPW_coverage,WP_ctrl_IPW_coverage,WR_coverage,WD_coverage)
res = data.frame(wp_true,wp_est,wp_sd,wp_se,wp_cr)
res
write.csv(res,file="C:/Users/82655/Dropbox/research/covariate adjustment win ratio/IPW_b_200.csv")  
